...
A Terraform module for creating `Compute` in VKCS (VK Cloud Solutions).
...
| <a name="input_server_group"></a> [server\_group](#input\_server\_group) | Configuration for creating a server group | <pre>object({<br/>    name        = string<br/>    policies    = optional(set(string))<br/>    region      = optional(string)<br/>    value_specs = optional(map(string))<br/>  })</pre> | n/a | yes |
...
| <a name="input_backup"></a> [backup](#input\_backup) | Configuration for backup plan. If null, backup plan will not be created | <pre>object({<br/>    name = string<br/>    schedule = object({<br/>      date        = optional(list(string))<br/>      every_hours = optional(number)<br/>      time        = optional(string)<br/>    })<br/>    max_full_backup = optional(string)<br/>    provider_name   = optional(string)<br/>  })</pre> | `null` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_backup_id"></a> [backup\_id](#output\_backup\_id) | Id of the backup plan. |
